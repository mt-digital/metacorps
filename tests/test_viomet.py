...
def _gen_test_input(pn, n, fw, so):
    ...
    rows = [
        ...
        (datetime(2016, 9, 2, 20), pn[1], n[0], fw[0], so[1], so[0]),
        (datetime(2016, 9, 2, 20), pn[1], n[0], fw[2], so[0], so[1]),
        (datetime(2016, 9, 2, 20), pn[1], n[0], fw[3], so[1], so[0]),

        # iCry Sad News on MSNBC says "kill" and "murder"
        # aired 9/2/16 at 18:00/6pm
        (datetime(2016, 9, 2, 18), pn[2], n[0], fw[0], so[1], so[2]),
        (datetime(2016, 9, 2, 18), pn[2], n[0], fw[1], so[3], so[0]),

        # Tracy Morgan on CNN says "kill" and "punch"
        # aired 9/1/16 at 21:00/9pm
        (datetime(2016, 9, 1, 21), pn[0], n[1], fw[0], so[0], so[2]),
        (datetime(2016, 9, 1, 21), pn[0], n[1], fw[2], so[0], so[3]),

        # Good morning, middle america! on Fox News says "kill" and "punch"
        # aired 9/1, 9/2, and 9/4 at 6:00
        (datetime(2016, 9, 1, 6), pn[4], n[2], fw[2], so[1], so[0]),

        (datetime(2016, 9, 2, 6), pn[4], n[2], fw[2], so[1], so[2]),

        # Digging Turnips on Fox News says "kill", "murder", and "attack"
        # aired 9/3 and 9/4 at 20:00/8pm
        (datetime(2016, 9, 3, 20), pn[3], n[2], fw[3], so[3], so[0]),
        (datetime(2016, 9, 3, 20), pn[3], n[2], fw[0], so[0], so[1]),

        (datetime(2016, 9, 4, 20), pn[3], n[2], fw[1], so[3], so[1]),
        (datetime(2016, 9, 4, 6), pn[4], n[2], fw[0], so[0], so[3]),
...
def test_count_for_each_local_time():
    ...
    pd.testing.assert_frame_equal(subject_output, expected_subject_output)
...
def test_partition_sum():
    '''
    Given a dataframe with counts for 'All' or network columns,
    calculate sum of each partition for each column
    '''
    partition_infos = {
...
def test_subject_object_counts_per_partition():
    ...
    date_range = pd.date_range('2016-09-01', '2016-9-4', freq='D')
    columns = pd.Index(
    ...
    pd.testing.assert_frame_equal(subj_trump.data_frame,
                                  expected_subj_trump_df)
